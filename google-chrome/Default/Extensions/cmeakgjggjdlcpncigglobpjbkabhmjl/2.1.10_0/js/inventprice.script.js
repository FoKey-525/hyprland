  ...
  setTimeout(function () {
    ...
    $J('.item_desc_description').on('click', '.floatbutton', function () {
        ...
        if (Array.isArray(CSGO_ORIGINS) && CSGO_ORIGINS.length && data.iteminfo) {
          ...
          if (data.iteminfo.globalRatingPos || data.iteminfo.localRatingPos) {
            ...
            if (data.iteminfo.globalRatingPos) {
              const $globalPositionBlock = $J(`
                <div class="item-position__global">
                      <span class="text">Global: </span>
                      <span class="value value_yellow">
                          ${data.iteminfo.globalRatingPos > 0 ? `${data.iteminfo.globalRatingPos} low` : `${Math.abs(data.iteminfo.globalRatingPos)} high`}
                      </span>
                </div>
              `).css('display', 'flex');
      ...
      if (!rgItem.floatvalue) {
        ...
        chrome.runtime.sendMessage(SIHID, { type: 'floatvalue', data: { link: actionLink } }, function (data) {
          updateFloatValue(data);
        });
      } else {
        const { floatvalue, paintseed, origin, globalRatingPos, localRatingPos } = rgItem;
        updateFloatValue({ iteminfo: { floatvalue, paintseed, origin, globalRatingPos, localRatingPos } });
      }
    });
...
var CalculateSellingPrice = function (basePrice) {
  ...
  if (adjustInput.getValue() === 0 && QuickSellAllFeature.isRunning && typeof window.fastdelta !== 'undefined') {
...
const GetFloat = (link, isForce = false) => {
  ...
  return new Promise((resolve, reject) => {
    chrome.runtime.sendMessage(SIHID, { type: 'floatvalue', data: { link, isForce } }, (respData) => {
