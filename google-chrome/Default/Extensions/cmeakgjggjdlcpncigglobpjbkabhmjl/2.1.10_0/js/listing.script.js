...
const floatData = {};
const floatQueue = [];
let stopListingFloatQueue = false;
  ...
  if (data && data.success && $item[0]) {
    ...
    if (data.iteminfo.globalRatingPos || data.iteminfo.localRatingPos) {
      ...
      if (data.iteminfo.globalRatingPos) {
        floatDiv.find('.item-position .item-position__global').css('display', 'flex').find('.value').html(`
        ${
      ...
      if (data.iteminfo.localRatingPos) {
        floatDiv
          ...
          .css('display', 'flex')
          .find('.value')
          .html(`${data.iteminfo.localRatingPos}`);
      }
    }
...
var setStickersPriceValues = async function (data) {
  ...
  if (countCallFloat === lengthRows) {
    ...
    const $column3ControlPanel1 = $J('.sih.control_panel_for_table_unique_inventories').find(
      '.control_panel_1 .block_3'
    );
...
var GetFloat = function (listingId, link, isForce = false) {
  return new Promise((resolve, reject) => {
    if (floatData[listingId].success && !isForce) {
      resolve(Object.assign({}, floatData[listingId], { listingId }));
    } else {
      chrome.runtime.sendMessage(SIHID, { type: 'floatvalue', data: { link, isForce } }, function (respData) {
...
const processFloatQueue = function () {
  ...
  Promise.all(
    floatQueue.map(({ listingId, inspectLink, currency, price, isFloat = true, stickersCount }, index) => {
      setTimeout(
        () => {
          if (stopListingFloatQueue) return;
          GetFloat(listingId, inspectLink)
            .then((data) => {
              if (data && data.iteminfo && data.iteminfo.stickers.length !== stickersCount) {
                GetFloat(listingId, inspectLink, true).then((data) => {
                  processFloatData(data, isFloat);
                });
              } else {
                processFloatData(data, isFloat);
              }
...
const processFloatData = (data, isFloat) => {
  if (isFloat) {
    setFloatValue(data);
