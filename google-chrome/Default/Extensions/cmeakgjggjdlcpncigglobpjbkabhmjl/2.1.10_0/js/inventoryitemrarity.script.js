  ...
  (() => {
    ...
    const setBackgroundColor = (item, $el, showPhase, cb = false) => {
      ...
      if (!phaseColor) {
        ...
        const tag = getTags(item).find(
          ({ category, internal_name }) => category === 'Rarity' && rules.includes(internal_name)
        );
        ...
        if (backgroundColor) {
          ...
          $el.parent().css({ 'background-color': 'white', 'background-image': 'none' });
          $el.addClass('colorized-background').css({ 'background-color': `#${backgroundColor}` });
        }
      }
    };
