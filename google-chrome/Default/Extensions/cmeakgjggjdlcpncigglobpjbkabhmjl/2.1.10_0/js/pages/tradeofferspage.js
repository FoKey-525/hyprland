/* global chrome */
var steamOfferExp =
  /<div class="link_overlay" onclick="ShowTradeOffer\( '(\d+)' \);"><\/div>[\s\S]+?<img src="(.+)">[\s\S]+?">([\s\S]+?)[：:]/g;
...
chrome.storage.sync.get(
  ...
  function (items) {
    ...
    $(function () {
      ...
      // chrome.action.setPopup({ popup: 'html/tradeoffers.html' });
...
const getRandomItem = (arr = []) => (arr.length ? arr[Math.round(Math.random() * 100) % arr.length] : null);
...
var nofSteam = function (idOffer, img, name) {
  ...
  var link = $(
    '<a href="https://steamcommunity.com/tradeoffer/' + idOffer + '/" target="_blank"><img src="' + path + '"></a>'
  );
  ...
  $(div).append(link);
  $(div).append(
    '<div class="offer-theirs"></div><div>for</div><div class="offer-yours"></div><div class="clearer">&nbsp;</div>'
  );
...
var AcceptTradeOffer = function (IdTradeOffer) {
  window.open(
    'https://steamcommunity.com/tradeoffer/' + IdTradeOffer + '/?sihaccept=' + sessionID,
    'HiddenTradeOffer' + IdTradeOffer,
    'height=10,width=10,resize=yes,scrollbars=yes'
  );
  return;
};
...
var RefuseTradeOffer = function (IdTradeOffer) {
  window.open(
    'https://steamcommunity.com/tradeoffer/' + IdTradeOffer + '/?sihrefuse=' + sessionID,
    'HiddenTradeOffer' + IdTradeOffer,
    'height=10,width=10,resize=yes,scrollbars=yes'
  );
  return;
...
var DeclineTradeOffer = function (tradeOfferID, g_sessionID) {
  ...
  request
    .done(function (data) {
      ...
      chrome.action.setBadgeText({ text: num ? '' + num : '' });
    ...
    .error(function (response) {
      ...
      if (response.status === 429 && response.responseText === '') {
        ...
        if (fatal.length) {
          ...
          offersDivBlock.append(
            'Steam: ' +
              errorMessage +
              '<br /><a href="' +
              tradeURL +
              '" target="_blank">Please, check your trade offers page.</a>'
          );
        }
      } else {
        errorMessage = 'Oops. We are sorry, but something went wrong';
    ...
    .done(function (data) {
      ...
      if (response && response.hasOwnProperty('trade_offers_received') && response.trade_offers_received.length) {
          ...
          $.ajax({
            ...
            .done(function (response) {
              ...
              if (name !== undefined && avatar !== undefined) {
                var header =
                  i18next.t('tradeoffers.offerfrom', { USERNAME: name.textContent }) +
                  (offer.message.length ? ': ' + offer.message : '');
                tradeofferdiv = nofSteam(offer.tradeofferid, avatar.textContent, header);
              } else {
                tradeofferdiv = nofSteam(
                  offer.tradeofferid,
        ...
        if (active_offers.length) {
          offersDivBlock.prepend(
            '<div style="margin-bottom:20px;" id="countoffers"><span id="title">' +
              i18next.t('tradeoffers.countoffers', { NUMBERS: '' + active_offers.length }) +
              '</span><br /> <a href="#" id="lnk_openall">' +
              i18next.t('tradeoffers.openall') +
              '</a> <a href="#" id="lnk_declineall">' +
              i18next.t('tradeoffers.declineall') +
              '</a> </div>'
          );
      ...
      chrome.action.setBadgeText({ text: active_offers.length ? '' + active_offers.length : '' });
      chrome.action.setBadgeBackgroundColor({ color: 'rgba(0, 0, 0, 0)' });
    })
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (itemsToReceive.length) {
        $('.offer-' + offerId + ' div:first').append(
          '<span class="offer-price">' +
            i18next.t('tradeoffers.theirprice') +
            ': <span class="price-theirs-' +
            offerId +
            '"></span></span>'
        );
      }
      if (itemsToGive.length) {
        $('.offer-' + offerId + ' div:first').append(
          '<span class="offer-price">' +
            i18next.t('tradeoffers.yourprice') +
            ': <span class="price-yours-' +
            offerId +
            '"></span></span>'
        );
      }
