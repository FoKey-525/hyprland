...
if (IS_ENABLED_SIH) {
  ...
  CInventory.prototype.LoadMoreAssets = function (count) {
    ...
    if (g_isLocalReloadAllInventory && +this.appid === +g_ActiveInventory.appid) {
      ...
      return this.LoadMoreAssetsMaster().done(() => {
        loadAllInventory();
      });
    }
  };
...
function loadInstantSellForInventoryItem(rgItem) {
  if (g_ActiveInventory.selectedItem && !$J(g_ActiveInventory.selectedItem.element).data('selling')) {
    ...
    if (!bIsTrading && window.instantsellbuttons && !selectmode) {
...
async function sellBtn() {
  ...
  if (+g_ActiveInventory.appid === 753) {
    setTimeout(async () => {
      ...
      $J('.sih_inventory_panel .sale_container .sih_turn_info_gems_inventory_button').html(
        SIHLang.inventory.title19.replace('$1', gooValue)
      );
    }, 500);
  }
...
const addInstantSellButton = (item, elActions) => {
  ...
  var instantSellBt = CreateMarketActionButton(
    'green',
    'javascript:void(0);',
    SIHLang.instantsell.replace('$1', '---')
  );
  ...
  const marketListingsURL =
    window.location.protocol +
    '//steamcommunity.com/market/listings/' +
    item.appid +
    '/' +
    encodeURIComponent(strMarketName);
  ...
  RequestCacher.get({
    ...
    chrome.runtime.sendMessage(
      SIHID,
      {
        type: 'BACKGROUND_GET_ORDER_HISTOGRAM',
        data: {
          g_strCountryCode,
          steamLang: 'english',
          g_rgWalletInfo,
          ...
          if (data.success && data.highest_buy_order) {
            ...
            if (g_ActiveInventory.selectedItem.description.market_hash_name === item.description.market_hash_name) {
              ...
              var instantSellBt = CreateMarketActionButton(
                'green',
                'javascript:void(0);',
                SIHLang.instantsell.replace('$1', sellingPrice)
              );
              instantSellBt.id = 'instantsellbtn';
