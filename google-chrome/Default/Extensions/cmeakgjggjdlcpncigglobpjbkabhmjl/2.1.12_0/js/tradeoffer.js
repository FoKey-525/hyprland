  ...
  function (items) {
    ...
    $.getJSON(chrome.runtime.getURL(`_locales/en/controls.json`), (enData) => {
      $.getJSON(chrome.runtime.getURL(`_locales/${items.lang}/controls.json`), (langData) => {
        ...
        const actualCode = [
          'window.offerdelayinterval = ' + (items.offerdelayinterval || '100') + ';',
          ...
          `window.IS_ENABLED_SIH = ${items.enabledSih_tradeofferPage}`,
        ].join('\r\n');
        ...
        chrome.storage.local.get(
          ...
          function (subitems) {
            const actualCode = ['window.bookmarkeditems = ' + JSON.stringify(subitems.bookmarks) + ';'].join('\r\n');
