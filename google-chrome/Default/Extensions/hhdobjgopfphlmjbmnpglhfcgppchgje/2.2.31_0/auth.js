...
;// CONCATENATED MODULE: ./src/background/auth/socialAuthSchema.ts


/**
 * This is a schema for the data that is returned from the social auth endpoint.
 */
var socialAuthUnderScoreSchema = z.object({
 ...
 */
var socialAuthSchema = socialAuthUnderScoreSchema.transform(function (data) {
  return {
...
;// CONCATENATED MODULE: ./src/content-scripts/auth.ts
...
function getParsedSocialAuth() {
  ...
  var data = lib_default().parse(queryString);
  try {
...
function documentReadyHandler() {
  ...
  var token = node && node.getAttribute('data-token');
  var newUser = node && node.getAttribute('data-new-user') || false;
  var redirectUrl = node && node.getAttribute('data-redirect-url');
  if (redirectUrl && isAbsolutePath(redirectUrl)) {
    var baseUrl = window.location.origin;
    redirectUrl = baseUrl + redirectUrl;
  }
  thankYouPageAuthHandler({
    token: token,
 ...
 */
function socialAuthHandler(authData) {
  browser_polyfill_default().runtime.sendMessage({
 ...
 */
function main() {
  var socialAuthData = getParsedSocialAuth();
  if (socialAuthData) {
